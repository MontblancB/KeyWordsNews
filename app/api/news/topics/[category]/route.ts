...
export async function GET(
  ...
  try {
    ...
    const cacheKey = `news:topic:${category}:hybrid${sourcesParam ? ':' + sourcesParam : ''}`

    console.log(`🔍 하이브리드 카테고리 조회 시작: "${category}"`)

    // 캐시 확인 (5분)
    const cached = cache.get<any[]>(cacheKey)
    if (cached) {
      console.log(`✅ 캐시에서 반환: ${cached.length}건`)
    ...
    // 3. 최신순 정렬 후 상위 50개만 반환
    const top50 = allResults.slice(0, 50)

    // 캐시에 저장 (5분)
    cache.set(cacheKey, top50, 300)

    console.log(`✅ 최종 결과 반환: ${top50.length}건`)
