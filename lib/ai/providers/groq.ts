...
export class GroqProvider implements AIProvider {
  ...
  async summarize(title: string, content: string): Promise<SummaryResult> {
    const prompt = `다음 뉴스 기사를 읽고 핵심 내용을 **3-5개의 초압축 불릿**으로 정리하고, 주요 키워드 3-5개를 추출해주세요.
        ...
        messages: [
          {
            role: 'system',
            content:
              '당신은 뉴스 핵심만 초압축하는 전문 AI입니다. 각 불릿은 15-20단어 이내로 극도로 압축하며, 핵심 키워드 중심으로 작성합니다. 숫자/날짜/금액 등 구체적 정보를 우선하고, 불필요한 조사/연결어/감정 표현은 제거합니다. 명사형/체언 중심 서술로 간결하게 작성합니다. JSON 형식으로 응답하며, summary는 "• 압축포인트1\\n• 압축포인트2" 형식입니다.',
          },
          {
            role: 'user',
            content: prompt,
