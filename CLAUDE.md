...
- 📝 **프롬프트 통합 관리**: Groq/Gemini/OpenRouter에 하드코딩된 프롬프트를 `lib/ai/prompts/summarize.ts`로 통합
- 🇰🇷 **한국어 규칙 현실화**: 비자연스러운 순우리말 강제 제거, 일상적 한자어 허용, 전문 용어 괄호 설명 방식으로 전환
- 📊 **카테고리별 예시 다양화**: 최저임금 예시 1개 과적합 → 10개 카테고리별 도메인 특화 예시 제공
- 🔑 **키워드 추출 기준 강화**: 구체적 선정 기준 명시 (이슈 키워드 우선, 일반적 단어 지양)
- 📌 **불릿 포인트 유연화**: 3-4개 고정 → 3-5개 본문 복잡도 기반 조절
- 🔧 **SummarizeOptions 타입 추가**: AIProvider 인터페이스에 category 옵션 지원 (하위 호환)
...
- 💡 **범례 안내**: 잠정실적 포함 시 하단에 설명 문구 표시

### v2.38.2 (2026-02-13) - 재무제표 최신 연도 표시 수정
- 📊 **최신 데이터 우선 표시**: 재무제표가 2020~2023만 보이던 문제 수정 → 최근 4개 연도 표시 (2022~2025)
- 🔄 **정렬 로직 개선**: `recentFinancials` 변수로 period 내림차순 정렬 후 최근 4개 선택
...
- 🔥 **TrendNow 기능**: 실시간 뉴스 키워드 트렌드 상위 20개 표시
- 📊 **자체 분석 엔진**: 최근 24시간 뉴스 200개에서 AI 키워드 추출
- ⏱️ **시간 가중치**: 최근 뉴스일수록 높은 가중치 적용 (빈도 기반)
- 🎯 **서비스 특화**: 자체 뉴스 데이터 활용 (외부 API 의존성 없음)
