...
8. **실시간 날씨** - 네이버 날씨 크롤링으로 기온, 미세먼지 등 실시간 확인
...
#### 추가 정보
- ☀️ **자외선 지수**: UV 수치 + 등급 (낮음/보통/높음/매우높음) 표시
- 🌅 **일출/일몰**: 일출·일몰 시간 정보 추가
- 🌫️ **오존 농도**: 오존 수치 + 대기질 등급 표시
- 🏜️ **황사 정보**: 황사 감지 여부 및 농도 레벨 (선택적)

#### 기술적 구현
- 📄 **types/weather.ts**: UV, SunriseSunset, Ozone 타입 추가
- 🕷️ **lib/scraper/naver-weather.ts**: 추가 정보 크롤링 로직 확장
- 🧩 **components/WeatherDetailModal.tsx**: UVCard, SunCard 컴포넌트 추가
- 🎨 **UI 확장**: 대기질 섹션 2x4 그리드, 일출/일몰 섹션 추가

#### 주요 특징
- 🔄 **선택적 표시**: 네이버에서 데이터를 찾을 때만 표시
- 🛡️ **안정성**: 데이터 없어도 에러 없이 정상 작동
- 🌓 **다크모드**: 모든 새 컴포넌트 완전한 다크모드 지원
- 🎯 **확장 가능**: 향후 시간대별/주간 예보 추가 준비
...
#### 날씨 정보 통합
- ☁️ **네이버 날씨 크롤링**: Cheerio를 활용한 실시간 날씨 데이터 수집
- 🌡️ **기본 날씨 정보**: 현재 기온, 체감온도, 습도, 강수확률, 풍속
- 🌤️ **날씨 상태**: 맑음/흐림/비/눈 아이콘 표시
- 🏭 **대기질 정보**: 미세먼지(PM10), 초미세먼지(PM2.5) + 등급 (좋음/보통/나쁨/매우나쁨)
- 📍 **지역**: 서울 고정 (향후 확장 예정)

#### UI/UX
- 🎨 **간결한 위젯**: 메인 페이지 헤더 오른쪽에 날씨 아이콘 + 기온 표시
- 🖱️ **상세 정보 모달**: 위젯 클릭 시 깔끔한 모달로 전체 정보 확인
- 🌓 **다크모드 지원**: 모달 및 위젯 완전한 다크모드 적용
- ⚡ **빠른 로딩**: 10분 캐싱으로 즉시 표시

#### 기술적 구현
- 📄 **types/weather.ts**: 날씨 데이터 타입 정의
- 🕷️ **lib/scraper/naver-weather.ts**: 네이버 날씨 크롤러 (네이버 금융 패턴 재사용)
- 🔌 **app/api/weather/route.ts**: 날씨 API 엔드포인트 (10분 캐싱)
- 🪝 **hooks/useWeather.ts**: React Query 훅 (자동 갱신)
- 🧩 **components/WeatherWidget.tsx**: 간결한 날씨 위젯
- 🧩 **components/WeatherDetailModal.tsx**: 상세 정보 모달

#### 주요 특징
- 💾 **10분 캐싱**: 서버 부하 방지 및 빠른 응답
- 🔄 **자동 갱신**: 10분마다 자동 새로고침
- 🛡️ **폴백 시스템**: 크롤링 실패 시 캐시 데이터 또는 기본값 반환
- 🎯 **에러 핸들링**: 안정적인 서비스 제공
